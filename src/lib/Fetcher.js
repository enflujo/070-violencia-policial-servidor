...
import { fmtName, fmtBlueprinterTitles, isFunction } from './util';
...
class Fetcher {
    ...
    return this.db
      ...
      .then((allUrls) => {
        ...
        return allParts.map((parts) => {
          const bp = buildDesaturated(this.sheetId, this.sheetName, parts[0], parts[1]);
  ...
  _saveViaBlueprinter(tab, data, blueprinter) {
    const saturatedBp = blueprinter(this.sheetId, this.sheetName, tab, data);
...
class LocalFetcher extends Fetcher {
  constructor(db, name, bps, path) {
    console.log(db, name, bps, path);
    super(db, name, bps);
    this.path = path;
    this.update().then((res) => console.log(`${res ? 'Successful' : "Couldn't"} update ${name}`));
  }
