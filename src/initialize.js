...
export default (callback) => {
  return Promise.resolve()
    ...
    .then((fetchers) => {
      const config = R.zipObj(
        themFetchers.map((f) => f.name),
        fetchers
      );
    ...
    .catch((err) => {
      console.log(err);
      console.log(
        `ERROR: the server couldn't connect to all of the sheets you provided. Ensure you have granted access to ${process.env.SERVICE_ACCOUNT_EMAIL} on ALL listed sheets.`
      );
    });
};
